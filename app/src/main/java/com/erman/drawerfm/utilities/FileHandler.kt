...
fun getDirectoryData(files: List<File>): List<DirectoryData> {
    ...
    {
        fileModelList.add(DirectoryData(files[i].path, files[i].isDirectory, files[i].name, convertFileSizeToMB(files[i].length())/*, it.extension*/, files[i].listFiles()?.size
            ?: 0, Date(files[i].lastModified())))
    }
