...
class RenameDialog(val name: String?) : DialogFragment() {
    ...
    override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
        return activity?.let {
            ...
            name?.let { name -> editText.setText(name) } ?: let { editText.setText("") }
            ...
            builder.setMessage(getString(R.string.rename))
                .setPositiveButton(R.string.ok) { _, _ ->
                    ...
                    if (inputMethodManager.isActive) inputMethodManager.toggleSoftInput(0, InputMethodManager.HIDE_NOT_ALWAYS)
                }
