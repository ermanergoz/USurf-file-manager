package com.erman.usurf.directory.utils

const val SIMPLE_DATE_FORMAT_PATTERN: String = "dd MMMM | HH:mm:ss"
const val MARQUEE_REPEAT_LIM: Int = -1 // -1 for infinite
const val THUMBNAIL_IMAGE_HEIGHT: Int = 100
const val THUMBNAIL_IMAGE_WIDTH: Int = 100
const val THUMBNAIL_SIZE_MULTIPLIER: Float = 0.1F
const val DUMMY_FILE_NAME: String = "usurfIsRootCheck.txt"
const val DUMMY_FILE_NAME_WO_EXTENSION: String = "usurfIsRootCheck"
const val EXTRACTED_FOLDER_NAME_SUFFIX: String = "_exracted"
const val FOLDER_SUFFIX: Char = '/'
const val HIDDEN_FILE_PREFIX: Char = '.'
const val MIME_TYPE_ALL: String = "*/*"
const val BUFFER_SIZE: Int = 131072 // Ideal for most devices also considering the old devices
const val FILE_EXTENSION_SEPARATOR: String = "."
const val ROOT_COMMAND_WAIT_TIME_MS: Long = 50
const val KILOBYTE_DIVISOR: Double = 1024.0
const val MEGABYTE_DIVISOR: Double = 1024.0 * 1024.0
const val GIGABYTE_DIVISOR: Double = 1024.0 * 1024.0 * 1024.0
const val TERABYTE_DIVISOR: Double = 1024.0 * 1024.0 * 1024.0 * 1024.0
