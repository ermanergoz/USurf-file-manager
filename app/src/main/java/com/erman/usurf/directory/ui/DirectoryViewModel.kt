...
class DirectoryViewModel(
    ...
    fun onFileClick(file: FileModel) {
        ...
        if (isMultiSelectionMode) {
            ...
            if (newSelection.isEmpty()) {
                ...
                updateState { s -> applySelectionChange(s, newSelection, file, canRename) }
            }
        } else {
            if (file.isDirectory) {
                setPath(file.path)
                ...
                _uiEvents.value =
                    Event(DirectoryUIEvent.ShowDialog(DialogArgs.OpenFileActivityArgs(file.path)))
            }
        }
    }
    ...
    fun onFileLongClick(file: FileModel): Boolean {
        ...
        updateState { s ->
            val updated = applySelectionChange(s, newSelection, file, canRename = true)
            updated.copy(
                common =
                    updated.common.copy(
    ...
    fun clearMultipleSelection() {
        ...
        updateState { s ->
            s.copy(
                common = s.common.copy(selectedFiles = emptyList(), canRename = false),
                files = clearedFiles,
            )
        }
    }
    ...
    fun onFileCompressOkPressed(zipNameWithExtension: String) {
        ...
        updateState {
            it.copy(
                ...
                activeActionCount = it.activeActionCount + 1,
            )
        }
        _uiEvents.value = Event(DirectoryUIEvent.ShowSnackbar(R.string.compressing))
        launch {
            try {
                directoryModel.compressFiles(selection, zipNameWithExtension)
                refreshFileList()
                _uiEvents.value = Event(DirectoryUIEvent.ShowSnackbar(R.string.compressing_successful))
            } catch (err: CancellationException) {
                _uiEvents.value =
                    Event(DirectoryUIEvent.ShowSnackbar(R.string.error_while_compressing))
                err.localizedMessage?.let { loge(it) } ?: UNKNOWN_ERROR
            } finally {
                updateState {
                    it.copy(
    ...
    fun extract() {
        ...
        _uiEvents.value = Event(DirectoryUIEvent.ShowSnackbar(R.string.extracting))
        launch {
            try {
                directoryModel.extractFiles(selection.first())
                refreshFileList()
                _uiEvents.value = Event(DirectoryUIEvent.ShowSnackbar(R.string.extracting_successful))
            } catch (err: CancellationException) {
                _uiEvents.value = Event(DirectoryUIEvent.ShowSnackbar(R.string.error_while_extracting))
                err.localizedMessage?.let { loge(it) } ?: UNKNOWN_ERROR
            } finally {
                updateState {
                    it.copy(
    ...
    fun showFileInformation() {
        ...
        selection.forEach { file ->
            _uiEvents.value =
                Event(DirectoryUIEvent.ShowDialog(DialogArgs.InformationDialogArgs(file)))
        }
    }
    ...
    fun share() {
        val selection = requireNonEmptySelectionAndResetPanel() ?: return
        _uiEvents.value =
            Event(DirectoryUIEvent.ShowDialog(DialogArgs.ShareActivityArgs(selection)))
    }
    ...
    fun onRenameOkPressed(fileName: String) {
        ...
        _uiEvents.value = Event(DirectoryUIEvent.ShowSnackbar(R.string.renaming))
        launch {
            try {
                directoryModel.rename(fileToRename, fileName)
                refreshFileList()
                _uiEvents.value = Event(DirectoryUIEvent.ShowSnackbar(R.string.renaming_successful))
            } catch (err: CancellationException) {
                _uiEvents.value = Event(DirectoryUIEvent.ShowSnackbar(R.string.error_while_renaming))
                err.localizedMessage?.let { loge(it) } ?: UNKNOWN_ERROR
            } finally {
                updateState {
                    it.copy(
    ...
    fun rename() {
        ...
        _uiEvents.value =
            Event(DirectoryUIEvent.ShowDialog(DialogArgs.RenameDialogArgs(selection.last().name)))
    }
    ...
    fun delete() {
        ...
        _uiEvents.value = Event(DirectoryUIEvent.ShowSnackbar(R.string.deleting))
        launch {
            try {
                directoryModel.delete(selection.toMutableList())
                refreshFileList()
                _uiEvents.value = Event(DirectoryUIEvent.ShowSnackbar(R.string.deleting_successful))
            } catch (err: CancellationException) {
                _uiEvents.value = Event(DirectoryUIEvent.ShowSnackbar(R.string.error_while_deleting))
                err.localizedMessage?.let { loge(it) } ?: UNKNOWN_ERROR
            } finally {
                updateState {
                    it.copy(
    ...
    fun onFolderCreateOkPressed(folderName: String) {
        ...
        updateState {
            it.copy(
                actionType = ActionType.CREATE_FOLDER,
                activeActionCount = it.activeActionCount + 1,
            )
        }
        _uiEvents.value = Event(DirectoryUIEvent.ShowSnackbar(R.string.creating))
        launch {
            try {
                directoryModel.createFolder(path, folderName)
                refreshFileList()
                _uiEvents.value =
                    Event(DirectoryUIEvent.ShowSnackbar(R.string.folder_creation_successful))
            } catch (err: CancellationException) {
                _uiEvents.value =
                    Event(DirectoryUIEvent.ShowSnackbar(R.string.error_when_creating_folder))
                err.localizedMessage?.let { loge(it) } ?: UNKNOWN_ERROR
            } finally {
                updateState {
                    it.copy(
                        actionType = ActionType.BROWSE,
                        activeActionCount = it.activeActionCount - 1,
                        common = it.common.copy(isCreateMenuExpanded = false),
                    )
                }
            }
        }
    ...
    fun onFileCreateOkPressed(fileName: String) {
        ...
        launch {
            try {
                ...
                refreshFileList()
                _uiEvents.value =
                    Event(DirectoryUIEvent.ShowSnackbar(R.string.file_creation_successful))
            } catch (err: CancellationException) {
                _uiEvents.value =
                    Event(DirectoryUIEvent.ShowSnackbar(R.string.error_when_creating_file))
                err.localizedMessage?.let { loge(it) } ?: UNKNOWN_ERROR
            } finally {
                updateState {
                    it.copy(
    ...
    fun onFavoriteButtonPressed() {
        ...
        _uiEvents.value =
            Event(DirectoryUIEvent.ShowDialog(DialogArgs.AddFavoriteDialogArgs(file.path)))
    }
