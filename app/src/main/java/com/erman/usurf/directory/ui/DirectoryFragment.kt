...
private const val MULTIPLE_FILES_THRESHOLD_FOR_SHARE_MESSAGE = 1
...
class DirectoryFragment : Fragment() {
    ...
    private fun showCreateFolderDialog() {
        ...
        dialog.onFolderCreateOkPressedListener = OnFolderCreateOkPressedListener { directoryViewModel.onFolderCreateOkPressed(it) }
        dialogListener.showDialog(dialog)
    }

    private fun showCreateFileDialog() {
        val dialog = CreateFileDialog()
        dialog.onFileCreateOkPressedListener = OnFileCreateOkPressedListener { directoryViewModel.onFileCreateOkPressed(it) }
        dialogListener.showDialog(dialog)
    }

    private fun showCompressDialog() {
        val dialog = CompressDialog()
        dialog.onCompressOkPressedListener = OnCompressOkPressedListener { directoryViewModel.onFileCompressOkPressed(it) }
        dialogListener.showDialog(dialog)
    }
    ...
    private fun handleShare(args: DialogArgs.ShareActivityArgs) {
        ...
        if (messages.size > MULTIPLE_FILES_THRESHOLD_FOR_SHARE_MESSAGE) {
